...
int main( int argc, char* argv[]) {
  ...
  if( argc!=2 && argc!=3 && argc!=4 ) {
    std::cout << "USAGE: ./do2ndLevel_TTZgen [dataset] [inputfile=""] [flags=""]" << std::endl;
    exit(31);
  }

  std::string dataset(argv[1]);

  Ntp1Analyzer_TTZgen* na;

  if( argc<4 ) {
    na = new Ntp1Analyzer_TTZgen(dataset);
  } else {
    std::string flags(argv[3]);
    na = new Ntp1Analyzer_TTZgen(dataset, flags);
  }
